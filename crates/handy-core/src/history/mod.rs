...
use crate::audio::utils::save_wav_file;
use handy_platform::EventSink;

mod retention;
pub use retention::RecordingRetentionPeriod;
...
/// Configuration for history cleanup. Passed in by the host app.
#[derive(Clone, Debug)]
pub struct RetentionConfig {
    pub period: RecordingRetentionPeriod,
    /// Used only when `period == PreserveLimit`.
...
impl Default for RetentionConfig {
    fn default() -> Self {
        Self {
            period: RecordingRetentionPeriod::PreserveLimit,
            history_limit: 100,
        }
    }
}
...
impl HistoryManager {
    ...
    pub async fn save_transcription(
        ...
        self.event_sink
            .emit_json("history-updated", serde_json::Value::Null);
    ...
    pub fn cleanup_old_entries(&self) -> Result<()> {
        match self.retention_config.period {
            RecordingRetentionPeriod::Never => Ok(()),
            RecordingRetentionPeriod::PreserveLimit => {
                self.cleanup_by_count(self.retention_config.history_limit)
            }
        ...
        for (id, file_name) in entries {
            ...
            if file_path.exists() {
                if let Err(e) = fs::remove_file(&file_path) {
                    log::error!("Failed to delete WAV file {}: {}", file_name, e);
                } else {
    ...
    fn cleanup_by_time(&self, retention_period: RecordingRetentionPeriod) -> Result<()> {
        ...
        let cutoff_timestamp = match retention_period {
            RecordingRetentionPeriod::Days3 => now - (3 * 24 * 60 * 60), // 3 days in seconds
            RecordingRetentionPeriod::Weeks2 => now - (2 * 7 * 24 * 60 * 60), // 2 weeks in seconds
            RecordingRetentionPeriod::Months3 => now - (3 * 30 * 24 * 60 * 60), // 3 months in seconds (approximate)
    ...
    pub async fn delete_entry(&self, id: i64) -> Result<()> {
        ...
        if let Some(entry) = self.get_entry_by_id(id).await? {
            ...
            if file_path.exists() {
                if let Err(e) = fs::remove_file(&file_path) {
                    log::error!("Failed to delete audio file {}: {}", entry.file_name, e);
